...
def add_trip(request):
    ...
    if res['added']:
        Plan.objects.create(destination=request.POST['destination'], description=request.POST['description'], image=request.POST['image'], travel_date_from=request.POST['travel_date_from'], travel_date_to=request.POST['travel_date_to'], user_plans=User.objects.get(id=request.session['user_id']))
    else:
